...
exports.getAllKids = async (req, res, next) => {
  try {
    // Get user information from query parameters (not req.body in a GET request)
    ...
    const user = await User.findOne({ name, _id: id });
    ...
    // Return the list of kids from the document
    res.status(200).json({
      size: user.kids?.length || 0,
      kids: user.kids || [],
    });
  } catch (error) {
  ...
  try {
    ...
    const exists = await Kid.findOne({ code: code, name: name });
        ...
        const school = await User.findOne(
                { role : 'admin' , name : req.body.school}
        )
        console.log(school);
    ...
    const newKid = await Kid.create(req.body);
        ...
        const school = await User.findOneAndUpdate(
            ...
            {$pull : {'kids.name' : kid.name , 'kids.id' : kid._id}},
            {new : true}
        )
        // console.log(school.kids.);
